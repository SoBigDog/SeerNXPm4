...
#if defined FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET && FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET
    config->srcAddr = MEMORY_ConvertMemoryMapAddress(config->srcAddr, kMEMORY_Local2DMA);
    config->destAddr = MEMORY_ConvertMemoryMapAddress(config->destAddr, kMEMORY_Local2DMA);
 ...
 */
status_t EDMA_SubmitTransfer(edma_handle_t *handle, edma_transfer_config_t *config)
{
    assert(handle != NULL);
    assert(config != NULL);
